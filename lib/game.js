...
Game.prototype.drawChronometer = function (counter){
    ...
    this.ctx.fillText(`Time: ${counter}`,50,50);
}
...
Game.prototype.level4 = function (counter) {
    if (this.level === 3 && (counter % 2 === 0)) {
        this.moveBalls(this.matrixPlayer,"L");
        this.moveBalls(this.matrixGame);
    }
}
Game.prototype.moveBalls = function (arr,dir) {
    var speed = 50;
    var limit = 30;
    arr.board.forEach(function (row,Rowindex) {
        // row is a matrix
        row.forEach(function (ball) {
            if(dir === "L"){
                ...
                if(ball.x <= (this.canvas.width - limit)){
                    ball.x += speed;
                }else{
                    // speed *= -1;
                }
