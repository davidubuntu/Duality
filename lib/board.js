...
Board.prototype.generatePlayerBoard = function () {
    for (var row = 0; row < this.game.rows; row++) {
        ...
        for (var column = 0; column < this.game.columns; column++) {
            this.board[row][column] = (new Ball(this.game, row, column, 0,this.game.radius,this.game.ballSpace,this.game.xBoard,this.game.yBoard)); //value 1 ó 0 para las del game segun empiece    
        }
    }
}
...
Board.prototype.generateGameBoard = function () {
    for (var row = 0; row < this.game.rows; row++) {
        ...
        for (var column = 0; column < this.game.columns; column++) {
            this.board[row][column] = (new Ball(this.game, row, column+3, Math.floor(Math.random()*2),this.game.radius,this.game.ballSpace,this.game.xBoard,this.game.yBoard));
