...
function setCanvasDimensions() {
    ...
    const marginWidth =  window.innerWidth - 1440
    canvasDOMElement.setAttribute("height", window.innerHeight - marginWidth);
    canvasDOMElement.setAttribute("width", 1440);
}
function setTitleContainerDimensions (){
  const  titleDOMElement = document.getElementById('top-container');
  if(window.innerWidth > 1500){
  const marginTitle =  ((window.innerWidth - 1440) / 2)/2.25
  titleDOMElement.style.top= `${marginTitle}px`;
  }
}
